...
export function Fans({ setConfig: _setConfig }: {
  ...
  const syncSharedFanCurves = (_next: CurvesState) => {
  ...
  return (
    <div className="afc-scope">
      <PanelSection title="qcom-fan curves">
        <Hint
          label="Shared with Control Center"
          description={
            draft.runtimeMode === "manual" || draft.runtimeMode === "off"
              ? `Control Center (Home+A) is in ${draft.runtimeMode}. Edited curves apply again when that mode is selected.`
              : "This tab edits the Silent / Auto / Aggressive curves. It does not change the current mode."
          }
        />
      </PanelSection>
